...
#### 6. Kiểu dữ liệu `Instant` vs `LocalDateTime` không nhất quán

- `security.user`, `operation.*`: Dùng `Instant` (UTC) → **đúng** cho dữ liệu phân tán.
- `core.branch`: Dùng `LocalDateTime` → có thể gây **mơ hồ timezone** khi query cross-region.

**Đề xuất:** Chuẩn hoá toàn bộ về `Instant` (UTC) và cấu hình `spring.jpa.properties.hibernate.jdbc.time_zone=UTC` (đã
