...
public class ProjectionOutboxService {
    private static final String UPSERT_SQL = """
            INSERT INTO operation.projection_outbox (
                projection_type, projection_key, aggregate_type, aggregate_key,
                year_value, quarter_value, skill_level, payload,
            ...
            ON CONFLICT (projection_key) DO UPDATE SET
                revision = operation.projection_outbox.revision + 1,
                ...
                payload = CASE
                    WHEN operation.projection_outbox.revision = operation.projection_outbox.processed_revision
                        THEN EXCLUDED.payload
                    WHEN EXCLUDED.projection_type = 'LEADERBOARD_MEMBER'
                        THEN jsonb_build_object(
                            'membershipChanged',
                            COALESCE((operation.projection_outbox.payload ->> 'membershipChanged')::boolean, false)
                            OR COALESCE((EXCLUDED.payload ->> 'membershipChanged')::boolean, false)
                        )
                    ELSE EXCLUDED.payload
                END,
                status = CASE
                    WHEN operation.projection_outbox.status = 'PROCESSING' THEN 'PROCESSING'
                    ELSE 'PENDING'
                END,
                retry_count = 0,
                next_attempt_at = NOW(),
                locked_by = CASE
                    WHEN operation.projection_outbox.status = 'PROCESSING' THEN operation.projection_outbox.locked_by
                    ELSE NULL
                END,
                locked_at = CASE
                    WHEN operation.projection_outbox.status = 'PROCESSING' THEN operation.projection_outbox.locked_at
                    ELSE NULL
                END,
                dirty_since = CASE
                    WHEN operation.projection_outbox.revision = operation.projection_outbox.processed_revision THEN NOW()
                    ELSE operation.projection_outbox.dirty_since
                END,
                last_error = NULL,
                updated_at = NOW()
            """;
    ...
    @Transactional(propagation = Propagation.MANDATORY)
    public void markFitnessDirty(String studentCode, int year, int quarter, SkillLevel skillLevel) {
        markDirty(ProjectionType.LEADERBOARD_FITNESS,
                "leaderboard:fitness:%d:Q%d:%s:student:%s".formatted(year, quarter, skillLevel, studentCode),
                "STUDENT", studentCode, year, quarter, skillLevel, "{}");
    }
    ...
    private void markDirty(ProjectionType type, String projectionKey, String aggregateType, String aggregateKey,
                           Integer year, Integer quarter, SkillLevel skillLevel, String payload) {
        jdbcTemplate.update(UPSERT_SQL,
                type.name(), projectionKey, aggregateType, aggregateKey,
                year, quarter, skillLevel == null ? null : skillLevel.name(), payload);
    }
    ...
    public List<ProjectionJob> claimReadyJobs(int batchSize, String instanceId) {
        List<ProjectionJob> jobs = jdbcTemplate.query("""
                SELECT id, projection_type, projection_key, aggregate_key,
                       year_value, quarter_value, skill_level, payload::text,
                       revision, retry_count
                FROM operation.projection_outbox
                WHERE (
                ...
                AND NOT EXISTS (
                    SELECT 1
                    FROM operation.projection_scope_state scope_state
                    WHERE scope_state.rebuilding = TRUE
                      ...
                      AND (
                          ...
                          OR (projection_type = 'LEADERBOARD_FITNESS'
                              AND scope_state.scope_key = 'fitness|' || year_value || '|' || quarter_value || '|' || skill_level)
                ...
                rs.getString("skill_level") == null ? null : SkillLevel.valueOf(rs.getString("skill_level")),
        ...
        for (ProjectionJob job : jobs) {
            int updated = jdbcTemplate.update("""
                    UPDATE operation.projection_outbox
    ...
    public long countProcessingForScope(LeaderboardScope scope) {
        ...
        if (scope.type() == LeaderboardScope.Type.QUARTER) {
            count = jdbcTemplate.queryForObject("""
                    SELECT COUNT(*) FROM operation.projection_outbox
            ...
            count = jdbcTemplate.queryForObject("""
                    ...
                    WHERE status = 'PROCESSING'
                      AND (projection_type = 'LEADERBOARD_MEMBER'
                           OR (projection_type = 'LEADERBOARD_FITNESS'
                               AND year_value = ? AND quarter_value = ? AND skill_level = ?))
                    """, Long.class, scope.year(), scope.quarter(), scope.skillLevel().name());
        }
    ...
    public long countDead() {
        Long count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM operation.projection_outbox WHERE status = 'DEAD'", Long.class);
        return count == null ? 0L : count;
    }
